...
def getMatchingTemplates(path2script, hierarchy_ls):
    ...
    #LOG 13: Show that the Helm template matches were found in the file, and the count.
    logObj.info("LOG 13: Found {} Helm template matches in file '{}'".format(len(templ_list), path2script))
    return templ_list
...
def getValidTaints(  lis_template_matches ): 
    ...
    #LOG 14: Show that the taint list was created and the count.
    logObj.info("LOG 14: Valid taint relationships constructed: {}".format(len(taint_lis)))
    return taint_lis 
